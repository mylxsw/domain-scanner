...
func (e *ApiCallError) Error() string {
	...
	if len(e.Errors) > 0 {
		...
		parts = append(parts, fmt.Sprintf("errors=%v", errors))
	}
...
func NewNamecheapClient(cfg *NamecheapConfig, ratePerMin float64, timeoutSec float64) *NamecheapClient {
	if timeoutSec <= 0 {
		timeoutSec = 60.0
	}
...
func (c *NamecheapClient) call(ctx context.Context, command string, params map[string]string) (string, error) {
	...
	err := Retry(ctx, func() error {
		...
		if resp.StatusCode >= 400 {
			return &ApiCallError{Message: "HTTP error", HTTPStatus: resp.StatusCode}
		}
		...
		if status != "OK" || len(errs) > 0 {
			...
			return &ApiCallError{Message: msg, HTTPStatus: resp.StatusCode, Errors: errs}
		}
...
func (c *NamecheapClient) DomainsCheck(ctx context.Context, domains []string) ([]DomainCheckResult, error) {
	if len(domains) == 0 {
		return nil, nil
	}
	if len(domains) > 50 {
...
// GetPricing tries to get pricing from XML
func (c *NamecheapClient) GetPricing(root *etree.Element) map[string]map[string]string {
	ns := "http://api.namecheap.com/xml.response"
	pricing := make(map[string]map[string]string)
